...
## 1. 概述
本应用提供个人收支记录、预算管理、数据可视化、数据导出与备份/恢复等功能。当前已重构为多页面结构：主页只保留总资产、本月收入支出和收支趋势，其余功能通过顶部导航进入对应子页面。所有交互在浏览器中完成，数据以结构化对象存储并支持持久化与再次渲染。
...
| 模块 | 主要函数 | 说明 |
|------|----------|------|
| 初始化 | `init()` | 按页面绑定事件、渲染当前页面所需模块、初始化对应图表 |
...
## 8. 页面结构
- `index.html`：总览页，仅显示总资产、本月收入、本月支出和收支趋势。
- `transactions.html`：记账/交易页，包含新增交易、最近交易、全部交易列表和详情模态框。
- `analytics.html`：统计分析页，包含月度对比、收支比例、收入分类与支出分类图表。
- `settings.html`：预算/导出/备份页，包含月度预算、CSV/Excel 导出和备份恢复。
...
```
├── index.html          # 总览页
├── transactions.html   # 记账/交易页
├── analytics.html      # 统计分析页
├── settings.html       # 预算/导出/备份页
├── styles.css          # 样式与细节增强
├── app.js              # 全部业务逻辑与状态管理
├── README.md           # 使用与特性说明
